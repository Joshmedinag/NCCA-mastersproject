...
def test_analyze_aggregates_metrics_from_fake_reader(tmp_path: Path) -> None:
    ...
    assert reader.inspected == [frame_a, frame_b]
...
def test_analyze_reports_unsupported_deep_exr_without_reading_frame(tmp_path: Path) -> None:
...
def test_analyze_reports_inspection_errors_and_continues(tmp_path: Path) -> None:
    ...
    assert report.metrics_by_aov == {}
    assert len(report.findings) == 1
    assert report.findings[0].rule_id == "inspect"
