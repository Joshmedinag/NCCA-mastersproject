...
class _MetricAccumulator:
    ...
    def update(self, pixels: np.ndarray) -> None:
        ...
        self.non_black_count += int(np.count_nonzero(finite > self.non_black_threshold))
        self.luminance_sum += float(np.sum(finite))
...
def analyze(
    ...
    *,
    ...
    for frame_index, frame_path in enumerate(discovery.frames, start=1):
        try:
            inspection = reader.inspect(frame_path)
        except Exception as exc:
            findings.append(
                Finding(
                    rule_id="inspect",
                    severity=Severity.ERROR,
                    message=f"Could not inspect EXR: {exc}",
                    file=frame_path,
                )
            )
            if progress_callback is not None:
                progress_callback(frame_index, total_frames, f"Inspection failed: {frame_path.name}")
            continue
