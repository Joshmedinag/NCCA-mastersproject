...
def validate_empty_aov(
    ...
    for aov_name, metrics in report.metrics_by_aov.items():
        ...
        if metrics.has_non_finite:
            continue
        if metrics.max_luminance <= max_luminance and metrics.avg_luminance <= max_average:
            findings.append(
                Finding(
                    rule_id=definition.id,
                    severity=definition.severity,
...
def validate_near_empty_aov(
    ...
    for aov_name, metrics in report.metrics_by_aov.items():
        if not _supports_aov(definition, aov_name, categories):
            continue
        if metrics.has_non_finite or metrics.max_luminance <= empty_max_luminance:
            continue
        if metrics.non_black_ratio <= max_ratio and metrics.avg_luminance <= max_average:
            findings.append(
                Finding(
                    rule_id=definition.id,
                    severity=definition.severity,
